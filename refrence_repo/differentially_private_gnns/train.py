...
def make_subgraph_from_indices(
  ...
  subgraph_nodes = jnp.where(valid_mask[:, jnp.newaxis], subgraph_nodes, 0.) # 有效位置保持真实节点特征，无效位置特征置0

  # Add a dummy padding node.
  padding_node = len(subgraph_indices) # dummy之前的节点是0...K-1(共K个), dummy在K
  subgraph_nodes = jnp.concatenate(
      (subgraph_nodes, jnp.zeros((1, subgraph_nodes.shape[1])))) # 在节点特征矩阵末尾追加一行0作为dummy node的特征向量
  subgraph_indices = jnp.where(valid_mask, subgraph_indices, padding_node) # padding位置都统一指向dummy node

  # Remap indices to within the subgraph. 构建星型结构（所有的点都由root node指向其他点）
...
def train_and_evaluate(config,
  ...
  if config.differentially_private_training:
    ...
    graph = jax.tree.map(jnp.asarray, graph) # 换成jnp格式，为了放到gpu上
